...
pub extern "C" fn function_params() -> i32 {
    ...
    let _ = trace_num("UINT64 Value Len:", output_len as u64);
    let value = u64::from_le_bytes(buf);
    // trace the value
    let _ = trace_num("UINT64 Value:", value as u64);
    // as hex
...
pub extern "C" fn instance_params() -> i32 {
    ...
    // let num_drops = match amount {
    //     TokenAmount::XRP { num_drops } if num_drops > 0 => num_drops,
    //     _ => {
    //         let _ = trace("Invalid `amount` Parameter - must be positive XRP");
    //         return BAD_PARAM;
    //     }
    // };

    // if (num_drops > 1_000_000_000) {
    //     let _ = trace("`amount` Parameter Exceeds Maximum Faucet Limit of 1,000,000,000 Drops");
    //     return BAD_PARAM;
    // }
